...
from modules.region_to_hexagons.parse_utils import (
    read_tsv_queue,
    get_name_from_geojson,
)
...
def sql_insert(
    operation: int,
    db_name: str,
    rows: multiprocessing.Queue,
) -> None:
    if operation == 0:
        sql_writer = SQLWriter(db_name)
    elif operation == 1:
        sql_writer = SQLWriterCellsCount(db_name)
...
def h3_cells_bytes(h3_resolution: int, region_from_json: str) -> SQLRegion | None:
    try:
...
def h3_cells_count(h3_resolution: int, region_from_json: str) -> SQLRegionCellsCount:
    try:
...
def multiprocessing_run(
    db_name: str, path_to_regions_json: str, h3_resolution: int, operation: int
):
    # Create the queues
    lines_from_file = multiprocessing.Queue()
    sql_rows = multiprocessing.Queue()

    # Create sql writer depend on operation type
    if operation == 0:  # h3 shapes
        long_operation = h3_cells_bytes
    elif operation == 1:  # h3 count
        long_operation = h3_cells_count
    else:
        raise ValueError("Wrong operation value")
    ...
    with multiprocessing.Pool() as pool:
        for sql_row in pool.imap_unordered(
            partial(long_operation, h3_resolution), iter(lines_from_file.get, None)
        ):
...
def create_cells_count_db(path_to_regions_json: str, h3_resolution: int):
    db_name = f"{Path(path_to_regions_json).stem}-{h3_resolution}-count"
    remove_db(db_name)

    multiprocessing_run(db_name, path_to_regions_json, h3_resolution, 1)
...
def create_region_hexagons_db(path_to_regions_json: str, h3_resolution: int):
    ...
    multiprocessing_run(db_name, path_to_regions_json, h3_resolution, 0)
